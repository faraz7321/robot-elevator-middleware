...
const API_AUTH_LIMITED_TOKEN_ENDPOINT =
  process.env.API_AUTH_LIMITED_TOKEN_ENDPOINT ||
  `https://${API_HOSTNAME}/api/v1/oauth2/limited-token`;
...
export async function fetchAccessToken(
  ...
  try {
    ...
    console.error(
      'Check that you have set the CLIENT_ID and CLIENT_SECRET environment variables correctly.',
    );
...
export const fetchResources = async (
  ...
  const resources = (result.data as string[]).filter((resource) =>
    resource.startsWith(`${resourceType}:`),
  );
...
export async function openWebSocketConnection(
  ...
  return new Promise((resolve, reject) => {
    ...
    ws = new WebSocket(
      `${WEBSOCKET_ENDPOINT}?accessToken=${accessToken}`,
      WEBSOCKET_SUBPROTOCOL,
    );
    // } catch (error) {
    //console.error('Error while opening WebSocket connection');
    // reject(error)
    //}
...
export async function resumeSession(
  ...
  sessionId: string,
): Promise<WebSocketResumeSessionResponse> {
 ...
 */
export const validateClientIdAndClientSecret = (
  CLIENT_ID: string,
